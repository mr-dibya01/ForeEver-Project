...
const Collection = () => {
  ...
  const { products, showSearchBar, searchQuerry } = useContext(ShopContext);
  ...
  function applyFilter() {
    ...
    if(searchQuerry && showSearchBar) {
      productCopy = productCopy.filter((item) => item.name.toLowerCase().includes(searchQuerry.toLowerCase()));
  ...
  return (
    <div className={`flex flex-col sm:flex-row gap-1 sm:gap-10 pt-10 ${showSearchBar ? "" : "border-t"}`}>
        ...
        <div className={`border border-gray-300 pl-5 py-3 mt-6 ${showFilter ? "" : 'hidden'} sm:block`}>
          ...
          <div className='flex flex-col gap-2 text-sm font-light text-gray-700'>
            <p className='flex gap-2'>
              <input className='w-3' type="checkbox" id='Men' value={'Men'} onChange={toggleCategory}/> 
          ...
          filterProducts.length > 0 ?  
            <>
              <div className="flex justify-between text-base sm:text-2xl mb-4">
                ...
                {/* Product Sort */}
                <select className='border-2 border-gray-300 text-sm px-2' onChange={ (e) => setSortFilter(e.target.value)} name="" id="">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 gap-y-6">
                { filterProducts.map((item,idx) => (
                  <ProductItem key={idx} image={item.image} name={item.name} price={item.price} id={item._id}/>
                ))}
              </div>
            </>
            : <p className='bg-red-200 px-4 py-5 rounded-md text-xl-'>No product found!</p>
          }
      </div>
    </div>
  )
